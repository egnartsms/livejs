"""EventFd is an event-like object that can be passed to select().

The code is taken from https://github.com/palaviv/eventfd. Support for C module based on
Linux native eventfd was thrown out.
...
class BaseEventFd(object):
    ...
    This EventFd class implements the same functions as a regular Event but it has a file
    descriptor. The file descriptor can be accessed using the fileno function. This event
    can be passed to select, poll and it will block until the event will be set.
